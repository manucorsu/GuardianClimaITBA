...
def prompt_ia_vestir(username: str):
    ...
    if ultima_consulta is None:
        print(
            "Debés hacer una consulta de clima para que la IA haga su recomendación."
        )
        return
    ciudad = ultima_consulta["Ciudad"]
    dt = pretty_dt(ultima_consulta["FechaHoraCompleta"], reverse=True)
    print(
        f"Se le consultará a Gemini cómo deberías vestirte en {ciudad} si el clima está como estaba {dt} (tu última consulta)."
    )
